...
def create_city_demos_table(cur, conn):
    '''
    This function uses an API to get the demographics of the top 100 most dangerous US cities we found with web scraping. 
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS City_Demos (city_id INTEGER, total_pop INTEGER, female_pop INTEGER, male_pop INTEGER, foreign_pop INTEGER, med_age FLOAT, white_pop INTEGER, black_pop INTEGER, asian_pop INTEGER, latin_pop INTEGER, na_pop INTEGER)")
    ...
    for i in range(25):
        ...
        #correction for the different spellings of 'saint' + Danville
        '''if city == "Danville" or city == "Saginaw" or city == "Pine Bluff" or city == "Alexandria" or city == "Monroe":
            continue'''
        ...
        cur.execute('INSERT INTO City_Demos (city_id, total_pop, female_pop, male_pop, foreign_pop, med_age) VALUES (?,?,?,?,?,?)', (city_id, total_pop, female_pop, male_pop, foreign_pop, med_age))
        conn.commit()
